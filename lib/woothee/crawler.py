...
def challenge_google(ua, result):
    ...
    if 'Mediapartners-Google' in ua:
        if 'compatible; Mediapartners-Google' in ua \
                or ua == 'Mediapartners-Google':
...
def challenge_crawlers(ua, result):
    if 'Yahoo' in ua or 'help.yahoo.co.jp/help/jp/' in ua \
            or 'listing.yahoo.co.jp/support/faq/' in ua:
        ...
        if 'crawler (http://listing.yahoo.co.jp/support/faq/' in ua\
                or 'crawler (http://help.yahoo.co.jp/help/jp/' in ua:
    ...
    if 'Baidu' in ua:
        if 'compatible; Baiduspider' in ua or 'Baiduspider+' in ua\
                or 'Baiduspider-image+' in ua:
    ...
    if 'ichiro' in ua:
        if 'http://help.goo.ne.jp/door/crawler.html' in ua or\
                'compatible; ichiro/mobile goo;' in ua:
    ...
    if 'Hatena ' in ua:
        if 'Hatena Antenna' in ua or 'Hatena Pagetitle Agent' in ua\
                or 'Hatena Diary RSS' in ua:
...
def challenge_maybe_crawler(ua, result):
    ...
    target_regex = re.compile('(?:Rome Client |UnwindFetchor/|'
                              'ia_archiver |Summify |PostRank/)')

    if target_regex.search(ua) or 'ASP-Ranker Feed Crawler' in ua:
        util.update_map(result, dataset.get('VariousCrawler'))
        return True
